...
- contract: clear - local-ledger is a product backlog adapter that maps a
...
- verify: `git diff --check` -> passed; `UV_CACHE_DIR=/private/tmp/smda-uv-cache
  uv run pytest packages/scheduler/tests/test_packaging.py -q` -> 12 passed.
  Focused local-ledger adapter contract tests belong to
