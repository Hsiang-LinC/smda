...
- contract: needs-human - decide whether the Codex plugin vendors the scheduler
  runtime, installs/points at a wheel, or keeps `smda-scheduler` as an external
  PATH prerequisite.
- acceptance: the chosen packaging contract states what plugin installation
  guarantees for `smda-scheduler mcp`, how version compatibility is checked, and
  what remains an operator prerequisite.
- verify: contract/doc tests plus a clean-plugin smoke check proving the MCP
  command either runs from the plugin-provided runtime or fails with the
  documented prerequisite message.
- next: choose runtime distribution mode.
- updated: 2026-06-24
