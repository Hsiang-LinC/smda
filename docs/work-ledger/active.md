...
- verify: local `darwin-x86_64` PyInstaller build plus bundled CLI/schema and newline MCP initialize smoke checks passed; packaging 29 passed; normalized full suite 442 passed, 1 skipped; TypeScript 16 passed, 1 skipped; typecheck and diff check passed. Workflow run `31163767307` failed before build because GitHub could not resolve `astral-sh/setup-uv@v9`; the workflow now pins the official immutable v8.1.0 commit and its regression/packaging tests pass. The literal full-suite command first reported 425 passed, 1 skipped, 11 failed, 6 errors because this host uses Git `main` while legacy tests assume `master`, and its 256-FD limit exhausts existing SQLite handles; isolated Git tests pass with a temporary `init.defaultBranch=master`, and the full suite passes with that config plus `ulimit -n 1024`.
- next: rerun the manually dispatched GitHub Actions workflow, then review native arm64/x86_64 artifacts before human acceptance
- updated: 2026-08-07
