...
- contract: boundary clear - implement as a product Tier-2 backlog adapter in
  `packages/scheduler`, never as setup-skill output.
...
- verify: `UV_CACHE_DIR=/private/tmp/smda-uv-cache uv run pytest
  ...
  packages/scheduler/tests -q` -> 373 passed, 1 skipped.
