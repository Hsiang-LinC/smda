...
- [x] **Step 1: Failing test** (real git in `tmp_path`, mirror the existing
...
- [x] **Step 2: Implement** `land_parent_to_base(operation)` and
...
- [x] **Step 1: Failing tests** — a `ParentLandOperation`
...
- [x] **Step 2: Implement** the `parent_land_ledger` table (mirror
...
- [x] **Step 2: Implement** `load_roadmap_for_member` (scan
...
- [x] **Step 3: Parity gate** — existing final-accept tests still pass (no-op land
...
- [x] **Step 1: Failing test** (real git) — `probe_conflict(head=parent-integration,
...
- [x] **Step 2: Implement** `ConflictProbeResult` (frozen: `clean`,
...
- [x] **Step 1: Failing tests** — `FINAL_ACCEPT_READY` now routes through a
...
- [x] **Step 2: Implement** `ParentPhase.LANDING_CONFLICT_PROBING` and
...
- [x] **Step 2: Implement** `GitParentIntegration.rebase_onto_base(operation)` and
...
- [x] **Step 3: Parity gate** — full parent-flow tests green. Commit.
  **4b green boundary.**
...
- [x] **Step 1: Failing tests** — the roadmap-integration branch name is derived
...
- [x] **Step 2: Implement** branch derivation + create-if-absent on first member
...
- [x] **Step 2: Implement** the roadmap-tier Aggregate stage(s)
...
- [x] **Step 3: Parity gate** — full suite green. Commit. **4c green boundary.**
...
- [x] End-to-end: a standalone parent lands to `main` on `FINAL_ACCEPT`; a
...
- [x] End-to-end: two independent parents that textually conflict — the loser
  rebases, re-reviews, and lands; the cap escalates to `HUMAN_REVIEW_REQUIRED`
  when rebase cannot converge.
- [x] Confirm: with no integration configured, parent final-accept behaviour is
